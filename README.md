...
```bash
PLAYWRIGHT_BROWSERS_PATH=.browsers requirements-fetcher analyze examples/github-issues.yaml
```

The CLI loads `GEMINI_API_KEY` from `.env`. An exported environment variable takes precedence. You can also run the package as `python -m requirements_fetcher`.
